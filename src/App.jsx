...
// Nippon Colors Theme (?•Êú¨?Æ‰?Áµ±Ëâ≤)
// Primary: Ê°úÈ? (Sakura-nezumi), ?ΩËó§ (Shiro-fuji)
// Accent: ?•Á´π (Wakatake), ?ÑÁ? (Usu-kurenai), ?§Ëâ≤ (Fuji-iro)
// Base: ?°Á? (Gofun), Â¢?(Sumi)
...
const COLOR_VARIANTS = {
  // Â¢®È? (Sumi-nezumi) - Default/Neutral - Ink gray
  slate: {
  ...
  // ?ÄÈº?(Gin-nezumi) - Secondary neutral
  stone: {
  ...
  // ?ΩËó§ (Shiro-fuji) - Pale wisteria lavender
  sky: {
  ...
  // ?ÑÁ? (Usu-kurenai) - Pale crimson/cherry
  rose: {
  ...
  // ?•Á´π (Wakatake) - Young bamboo green
  emerald: {
  ...
  // Ê°îÊ? (Kikyo) - Bellflower purple-blue
  indigo: {
...
const INCOME_CATEGORIES = ['?™Ê∞¥', 'Âπ¥Á??éÈ?', 'ÊøÄ?µÁ???, '?∂‰??éÈ?'];

const MENU_SECTIONS = [
  {
    title: 'Ë®òÂ∏≥?üËÉΩ',
    items: [
      { id: 'home', label: 'Â∏≥Â?Á∏ΩË¶Ω', icon: Home },
      { id: 'calendar', label: 'ÊØèÊó•?éÁ¥∞', icon: Calendar },
      { id: 'visualization', label: '?ØÂá∫?ÜÊ?', icon: BarChart2 },
      { id: 'income', label: '?∂ÂÖ•ÁÆ°Á?', icon: DollarSign },
      { id: 'settings', label: '?êÁ?Ë®≠Â?', icon: SettingsIcon },
    ]
  },
  {
    title: '?≤Ë??üËÉΩ',
    items: [
      { id: 'watchlist', label: 'ÂÆöÊ??çÂñÆ', icon: Layers },
      { id: 'stock_goals', label: 'Â≠òËÇ°Ë®àÁï´', icon: Target },
      { id: 'partner', label: '‰Ω≥Ê¨£?≤Ë?', icon: Users },
      { id: 'principal', label: 'Ë≥áÁî¢Ê∑®ÂÄ?, icon: PieChart },
      { id: 'mortgage', label: '?øÁî¢?ïË?', icon: Building2 },
    ]
  }
];
...
const formatDetailedDate = (dateStr) => {
  ...
  const weekdays = ['??, '‰∏Ä', '‰∫?, '‰∏?, '??, '‰∫?, '??];
  return `${date.getFullYear()}Âπ?{date.getMonth() + 1}??{date.getDate()}??(??{weekdays[date.getDay()]})`;
};
...
// --- Components ---
const ConfirmationModal = ({ isOpen, onClose, onConfirm, message, title = "Á¢∫Ë?", confirmText = "Á¢∫Â?", confirmColor = "bg-stone-800" }) => {
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-6">
      ...
      <div className={`relative ${GLASS_CARD} p-6 w-full max-w-xs animate-in zoom-in-95 duration-200`}>
        ...
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 py-3 bg-stone-100 text-stone-600 rounded-xl font-bold active:scale-95 transition-transform text-xs">?ñÊ?</button>
...
const GlassButton = ({ onClick, children, className = "", disabled = false, variant = 'primary', type = "button" }) => {
  // Nippon Colors: Primary uses ?§È? (Fuji-nezumi), a sophisticated grayish purple
  const variants = {
    ...
    <div className="w-full">
      <div className="flex justify-between items-end mb-2">
        <span className={`text-xs font-bold uppercase tracking-wider ${theme.text} opacity-80 flex items-center gap-2`}>
          {label}
          {isOverBudget && showOverBudgetLabel && <span className="bg-rose-50 text-rose-600 text-[10px] px-1.5 py-0.5 rounded-md shadow-sm">Â∑≤Ë???/span>}
        </span>
        {showDetails && (
          <div className="flex items-baseline gap-1 text-right">
            <span className={`text-[10px] font-medium whitespace-nowrap ${isOverBudget ? 'text-rose-400' : 'text-stone-400'}`}>{isOverBudget ? 'Â∑≤Ë??? : '?©È?'}</span>
      ...
      {showDetails && variant === 'main' && (
        <div className="flex justify-between mt-1.5 text-[10px] text-stone-400 font-medium">
          <span className={isOverBudget ? 'text-rose-400' : ''}>{Math.round(remainingPercentage)}% ?©È?</span>
          <span>Á∏ΩÈ?: ${total.toLocaleString()}</span>
        </div>
      )}
    </div>
  );
...
const PrincipalTrendChart = ({ history }) => {
  ...
  if (!data || data.length < 2) { return (<div className={`${GLASS_CARD} p-6 mb-6 flex flex-col items-center justify-center h-48`}><PieChart className="w-8 h-8 text-stone-300 mb-2" /><span className="text-xs text-stone-400 font-medium">Á¥ØÁ??¥Â?Á¥Ä?ÑÂ?È°ØÁ§∫Ë∂®Âã¢??/span></div>); }
      ...
      <div className="flex flex-col cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
        <div className="flex justify-between items-center mb-3">
          <div className="flex items-center gap-3">
            ...
            {isOverBudget && <span className="bg-rose-100 text-rose-600 text-[10px] px-1.5 py-0.5 rounded-full font-bold">Â∑≤Ë???/span>}
          </div>
          <div className="text-right flex items-baseline gap-1">
            <span className="text-[10px] text-stone-400">?©È?</span>
...
const WatchlistGroup = ({ group, onUpdateStock, onDeleteStock, onDeleteGroup, onAddStock, totalSystemBudget, prices }) => {
  ...
  return (
    <div className={`${GLASS_CARD} p-5 hover:border-indigo-300 transition-all duration-300 mb-4 ${theme.glow}`}>
      <div className="flex justify-between items-center mb-4"><div className="flex items-center gap-3 cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}><div className={`p-1.5 rounded-lg ${theme.iconBg} ${theme.iconText}`}>{isExpanded ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}</div><div><h3 className="text-sm font-bold text-stone-700 tracking-tight">{group.name}</h3><span className="text-[10px] text-stone-400 font-medium">‰ΩîÁ∏Ω?êÁ? {groupPercentage.toFixed(1)}%</span></div></div><div className="flex items-center gap-2"><button onClick={(e) => { e.stopPropagation(); onDeleteGroup(); }} className="p-1.5 rounded-lg bg-stone-100 text-stone-400 hover:bg-rose-50 hover:text-rose-500 transition-all"><X className="w-3.5 h-3.5" /></button></div></div>
      {isExpanded && (<div className="space-y-4 animate-in slide-in-from-top-1 duration-200 border-t border-stone-100/50 pt-3">{group.items.map((stock, idx) => { const priceData = prices[stock.symbol]; const budget = Number(stock.budget) || 0; const price = priceData ? priceData.price : 0; const shares = price > 0 ? Math.floor(budget / price) : 0; const isUp = priceData?.change >= 0; const stockPercentage = groupTotalBudget > 0 ? (budget / groupTotalBudget) * 100 : 0; return (<div key={idx} className="flex flex-col gap-2 border-b border-stone-100/50 last:border-0 pb-3 last:pb-0"><div className="flex justify-between items-center"><div><div className="flex items-center gap-2"><span className="font-bold text-stone-800 text-base">{stock.symbol}</span><span className="text-[10px] text-stone-400 bg-stone-100 px-1.5 rounded">‰ΩîÊ? {stockPercentage.toFixed(1)}%</span>{priceData && (<span className={`text-[10px] font-bold px-1.5 py-0.5 rounded flex items-center gap-0.5 ${isUp ? 'bg-emerald-100 text-emerald-600' : 'bg-rose-100 text-rose-600'}`}>{isUp ? <TrendingUp className="w-3 h-3" /> : <TrendingDown className="w-3 h-3" />}{priceData.change.toFixed(2)}%</span>)}</div><div className="text-xs text-stone-400 font-mono mt-0.5">?æÂÉπ: ${price > 0 ? price.toFixed(2) : '---'}</div></div><button onClick={() => onDeleteStock(group.id, idx)} className="text-stone-300 hover:text-rose-400 p-1"><X className="w-4 h-4" /></button></div><div className="bg-stone-50/50 rounded-xl p-3 flex items-center gap-3"><div className="flex-1"><label className="text-[10px] text-stone-400 font-bold uppercase block mb-1">ÂÆöÊ??êÁ? (USD)</label><input type="number" value={stock.budget} onChange={(e) => onUpdateStock(group.id, idx, 'budget', e.target.value)} className="w-full bg-white/50 border border-stone-200 rounded-lg px-2 py-1 text-sm font-bold text-stone-700 outline-none focus:border-indigo-300" placeholder="500" /></div><div className="text-right"><div className="text-[10px] text-stone-400 font-bold uppercase mb-1">?ØË≥º?°Êï∏</div><div className="text-xl font-bold text-indigo-600 font-mono">{shares} <span className="text-xs text-stone-400 font-sans">??/span></div></div></div></div>); })}<div className="mt-2 pt-2 border-t border-stone-100/50"><div className="flex gap-2"><input value={newSymbol} onChange={(e) => setNewSymbol(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAdd()} placeholder="Ëº∏ÂÖ•‰ª?¢º" className={`${GLASS_INPUT} py-2 px-3 text-xs uppercase`} /><button onClick={handleAdd} className="bg-stone-800 text-white px-4 rounded-xl hover:bg-stone-700 font-bold text-xs shadow-lg"><Plus className="w-4 h-4" /></button></div></div></div>)}
    </div>
  );
};
...
const WatchlistView = ({ user, db, appId, requestConfirmation }) => {
  ...
  const deleteGroup = (id) => requestConfirmation({ message: 'Á¢∫Â??™Èô§Ê≠§Áæ§ÁµÑÔ?', onConfirm: async () => { const newGroups = groups.filter(g => g.id !== id); await saveGroups(newGroups); } });
  ...
  return (
    <div className="pb-24 space-y-6 animate-in fade-in">
      <div className="flex justify-between items-end mb-2 px-2">
        <div><h2 className="text-xl font-bold text-stone-800">?ïË??çÂñÆ</h2><p className="text-xs text-stone-400 font-mono mt-1">{lastUpdated ? `?ÄÂæåÊõ¥?? ${lastUpdated.toLocaleTimeString()}` : '?¥Êñ∞‰∏?..'}</p></div>
      ...
      <div className="mb-4">
        {!isAddGroupOpen ? (
          <button onClick={() => setIsAddGroupOpen(true)} className="w-full py-3 border-2 border-dashed border-stone-200 rounded-2xl text-stone-400 font-bold text-sm hover:border-indigo-300 hover:text-indigo-500 transition-all flex items-center justify-center gap-2"><Plus className="w-4 h-4" /> ?∞Â?ËøΩËπ§Ê∏ÖÂñÆ</button>
        ) : (
          <div className={`${GLASS_CARD} p-3 flex gap-2 animate-in slide-in-from-top-2 duration-200`}><input value={newGroupName} onChange={e => setNewGroupName(e.target.value)} onKeyDown={e => e.key === 'Enter' && addGroup()} placeholder="Ëº∏ÂÖ•Áæ§Á??çÁ®±" className={`${GLASS_INPUT} py-2 px-3 text-xs uppercase`} autoFocus /><button onClick={addGroup} disabled={isSubmitting} className="bg-indigo-600 text-white px-4 rounded-xl font-bold shadow-md hover:bg-indigo-700"><Check className="w-4 h-4" /></button><button onClick={() => setIsAddGroupOpen(false)} className="bg-stone-100 text-stone-500 px-3 rounded-xl hover:bg-stone-200"><X className="w-4 h-4" /></button></div>
        )}
      </div>
...
const SalaryHistoryCard = ({ history, owner, onAdd, onDelete, onEdit }) => {
  const [isExpanded, setIsExpanded] = useState(false);
  return (<div className={`${GLASS_CARD} p-5`}><div className="flex justify-between items-center cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}><div className="flex items-center gap-3"><div className="p-2 bg-stone-100 rounded-xl text-stone-500"><Briefcase className="w-4 h-4" /></div><span className="font-bold text-stone-700 text-sm">?™Ë??êÈï∑Á¥Ä??/span></div><GlassButton onClick={(e) => { e.stopPropagation(); onAdd(owner); }} variant="ghost" className="px-2 py-1 text-xs">+ Ë™øËñ™</GlassButton></div>{isExpanded && (<div className="mt-4 space-y-3 pt-3 border-t border-stone-100/50">{history.length === 0 ? (<p className="text-xs text-stone-300 text-center py-2">Â∞öÁÑ°Ë™øËñ™Á¥Ä??/p>) : (history.map((rec, idx) => { const prevRec = history[idx + 1]; let percentChange = null; if (prevRec && prevRec.amount > 0) percentChange = ((rec.amount - prevRec.amount) / prevRec.amount) * 100; return (<div key={rec.id} onClick={() => onEdit && onEdit(rec)} className="flex justify-between items-center text-sm border-b border-stone-50 last:border-0 pb-2 last:pb-0 cursor-pointer hover:bg-stone-50 px-1 rounded transition-colors"><div className="flex flex-col"><span className="font-mono font-bold text-stone-700">${Number(rec.amount).toLocaleString()}</span><span className="text-[10px] text-stone-400">{rec.date}</span></div><div className="flex items-center gap-2">{percentChange !== null && (<span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${percentChange >= 0 ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-500'}`}>{percentChange >= 0 ? '+' : ''}{percentChange.toFixed(1)}%</span>)}<button onClick={(e) => { e.stopPropagation(); onDelete(rec.id); }} className="text-stone-300 hover:text-rose-400 p-1"><X className="w-3 h-3" /></button></div></div>); }))}</div>)}{!isExpanded && history.length > 0 && <div className="mt-2 text-xs text-stone-400 pl-11">?ÆÂ?: <span className="font-mono text-stone-600 font-bold">${Number(history[0].amount).toLocaleString()}</span></div>}</div>);
};
    ...
    <div className={`${GLASS_CARD} overflow-hidden mb-3`}>
      <div onClick={() => setIsExpanded(!isExpanded)} className="bg-stone-50/50 p-4 flex justify-between items-center cursor-pointer transition-colors hover:bg-stone-50/80">
        <div className="flex items-center gap-3">
          ...
          <span className="font-bold text-stone-700 text-sm">{year}Âπ¥Â∫¶</span>
        </div>
      ...
      {isExpanded && (<div className="p-2 space-y-2">{transactions.map(tx => (
        <div key={tx.id} onClick={() => onEdit(tx)} className="flex justify-between items-center p-3 bg-white/50 rounded-xl border border-stone-100 hover:border-stone-200 transition-all group cursor-pointer">
          <div className="flex gap-3 items-center">
            ...
            <div className="flex flex-col"><span className="font-bold text-stone-700 text-xs">{tx.type === 'saving' ? 'Â≠òÂÖ•' : '?ØÂá∫'}</span><span className="text-[10px] text-stone-400 flex items-center gap-1">{tx.date} {tx.note && `??${tx.note}`}</span></div>
          </div>
    ...
    <div className="space-y-3">
      {(items || []).map((item, idx) => (
        <div key={idx} className="flex items-center gap-2 animate-in slide-in-from-left-2 duration-200 min-w-0">
          <input value={item.name} onChange={(e) => onUpdate(section, groupKey, idx, 'name', e.target.value)} placeholder="?ÖÁõÆ?çÁ®±" className={`${GLASS_INPUT} flex-1 text-base py-2 px-3`} />
  ...
  return (
    <div className={`${GLASS_CARD} p-5 mb-4 relative overflow-hidden`}>
      ...
      <div className="flex justify-between items-start mb-4 pl-3">
        <div>
          <h3 className="text-lg font-bold text-stone-800 flex items-center gap-2">
            {yearData.year}Âπ?            {isAchieved ? (
              <span className="text-xs bg-emerald-50 text-emerald-600 px-2 py-0.5 rounded-full font-bold">?îÊ?</span>
            ) : (yearData.year < new Date().getFullYear()) ? (
              <span className="text-xs bg-rose-50 text-rose-500 px-2 py-0.5 rounded-full font-bold">?™È???/span>
            ) : (
              <span className="text-xs bg-stone-100 text-stone-400 px-2 py-0.5 rounded-full font-bold">?≤Ë?‰∏?/span>
            )}
            {currentWithdrawal > 0 && <span className="text-xs bg-amber-100 text-amber-600 px-2 py-0.5 rounded-full font-bold flex items-center gap-1">?´Ê???/span>}
          </h3>
          <div className="text-xs text-stone-400 mt-1">?∫Â?Â≠òÂÖ•: <span className="font-bold text-stone-600">${fixedDeposit.toLocaleString()}</span> (ÁæéÈ?)</div>
        </div>
        <div className="text-right">
          <div className="text-xs text-stone-400">Âπ¥Â??ÆÊ?</div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-4 pl-3">
        <div><label className="text-[10px] text-stone-400 uppercase font-bold">Firstrade (ÁæéÈ?)</label><input type="number" value={yearData.firstrade} onChange={(e) => onUpdate(yearData.id, 'firstrade', e.target.value)} className="w-full font-mono font-bold text-stone-700 border-b border-stone-100 focus:border-emerald-500 outline-none py-1 bg-transparent" placeholder="0" /></div>
        <div><label className="text-[10px] text-stone-400 uppercase font-bold">IB (ÁæéÈ?)</label><input type="number" value={yearData.ib} onChange={(e) => onUpdate(yearData.id, 'ib', e.target.value)} className="w-full font-mono font-bold text-stone-700 border-b border-stone-100 focus:border-emerald-500 outline-none py-1 bg-transparent" placeholder="0" /></div>
        <div className="col-span-2 relative"><label className="text-[10px] text-amber-400 uppercase font-bold">?êÈ?/Ë™øÁ? (ÁæéÈ?)</label><input type="number" value={yearData.withdrawal} onChange={(e) => onUpdate(yearData.id, 'withdrawal', e.target.value)} className="w-full font-mono font-bold text-stone-700 border-b border-amber-100 focus:border-amber-400 outline-none py-1 bg-transparent" placeholder="0" /></div>
      </div>
      <div className="bg-stone-50/50 rounded-xl p-3 pl-4 flex justify-between items-center">
        <div><div className="text-[10px] text-stone-400 mb-0.5 font-bold uppercase">?ÆÊ??ëÈ?</div><div className="font-bold text-stone-500 text-sm font-mono">${Math.round(targetAmount).toLocaleString()}</div></div>
        <div className="text-right"><div className="text-[10px] text-stone-400 mb-0.5 font-bold uppercase">ÂØ¶È?Á∏ΩË???/div><div className={`font-bold text-lg font-mono ${isAchieved ? 'text-emerald-600' : 'text-stone-700'}`}>${Math.round(currentTotal).toLocaleString()}</div><div className={`text-[10px] font-medium ${isAchieved ? 'text-emerald-500' : 'text-stone-400'}`}>Ë™§Â∑Æ: {diff > 0 ? '+' : ''}{Math.round(diff).toLocaleString()} ({errorPercent.toFixed(2)}%)</div></div>
      </div>
    </div >
  );
};
    ...
    <div className="flex items-center gap-3">
      ...
      <div>
        <div className="text-sm font-bold text-stone-700">
          Ë≤∑ÂÖ• ${Number(item.usdAmount).toLocaleString()}
        </div>
...
// Áµ±‰??ÑÂ?Ë°®Â?‰ª?const StandardList = ({ title, items, onDelete, onAdd, onEdit, icon: Icon, type, totalLabel, totalValue, itemRenderer, variant = 'slate', isCollapsible = false, defaultExpanded = true }) => {
  const theme = COLOR_VARIANTS[variant] || COLOR_VARIANTS.slate;
  ...
  return (
    <div className={`${GLASS_CARD} overflow-hidden p-0 mb-6 ${theme.glow}`}>
      ...
      >
        ...
        <div className="flex items-center gap-3">
          {(!isCollapsible || isExpanded) && <GlassButton onClick={(e) => { e.stopPropagation(); onAdd(type); }} className="text-xs px-2 py-1"><Plus className="w-3 h-3" /> ?∞Â?</GlassButton>}
      ...
      {isExpanded && (
        <div className="p-5 animate-in slide-in-from-top-2 duration-200">
          <div className="space-y-3">
            {items.length === 0 ? <p className="text-center text-xs text-stone-300 py-4">?°Á???/p> : items.map((item) => (
      ...
      {expandedYear && (
        <div className="grid grid-cols-2 gap-3 mb-4 animate-in slide-in-from-top-2 duration-200">
          <div className="bg-stone-50/80 p-3 rounded-xl border border-stone-100">
            <div className="text-xs text-stone-400 mb-1">‰∏ãÊ? ({currentStatus?.id}??</div>
            <div className="text-lg font-bold text-stone-800 font-mono">${currentStatus?.amount.toLocaleString()}</div>
            <div className="text-[10px] text-stone-400">?©Á? {currentStatus?.rate}%</div>
          </div>
          <div className="bg-stone-50/80 p-3 rounded-xl border border-stone-100">
            <div className="text-xs text-stone-400 mb-1">Á¥ØË?Â∑≤È?Ê¨?/div>
      ...
      {!expandedYear && (
        <div className="flex justify-between items-center px-2 pb-2">
          <div className="flex flex-col">
            <span className="text-[10px] text-stone-400 uppercase font-bold">‰∏ãÊ??ëÈ?</span>
            <span className="font-mono font-bold text-stone-600 text-sm">${currentStatus?.amount.toLocaleString()}</span>
          </div>
          <div className="flex flex-col items-end">
            <span className="text-[10px] text-stone-400 uppercase font-bold">Á¥ØË?Â∑≤È?</span>
          ...
          {Object.entries(yearlyGroups).map(([year, rows]) => (
            <div key={year} className="border border-stone-100 rounded-xl overflow-hidden">
              <div onClick={(e) => { e.stopPropagation(); setExpandedYear(Number(year) === expandedYear ? 'overview' : Number(year)); }} className="bg-white/80 p-3 flex justify-between items-center text-xs font-bold text-stone-600 cursor-pointer hover:bg-stone-50">
                <span>{year}Âπ¥Â∫¶ ({rows.length}??</span>
              ...
              {Number(year) === expandedYear && (
                <div className="bg-white/50 backdrop-blur-sm">
                  <div className="grid grid-cols-5 text-[10px] text-stone-400 px-3 py-2 border-b border-stone-100 bg-stone-50/50">
                    <span>?Ä??/span><span>?•Ê?</span><span className="text-center">?üÊï∏</span><span className="text-right">?ëÈ?</span><span className="text-right">?©Á?</span>
                  </div>
                  {rows.map(row => (
                    <div key={row.id} className={`grid grid-cols-5 text-xs px-3 py-2 border-b border-stone-100 last:border-0 items-center ${row.isPaid ? 'bg-emerald-50/30' : 'hover:bg-stone-50/30'}`}>
                      ...
                      <span className={`font-mono ${row.isPaid ? 'text-emerald-700 font-bold' : 'text-stone-500'}`}>{row.date.getMonth() + 1}??/span>
  ...
  return (
    <div className={`flex flex-col gap-4 w-full`}>
      <div className={`${GLASS_CARD} p-5 relative overflow-hidden`}>
        ...
        <div className="text-2xl font-bold text-stone-800 tracking-tight font-mono mb-1 relative z-10">${total.toLocaleString()}</div>
        <div className="text-[10px] font-bold text-stone-400 uppercase tracking-wider relative z-10">Âπ¥Â∫¶Á¥ØË??∂ÂÖ•</div>
      </div>
      <SalaryHistoryCard history={history} owner={owner} onAdd={onAddSalary} onDelete={onDeleteSalary} onEdit={onEditSalary} />
      <div className={`${GLASS_CARD} p-5`}>
        <div className="flex justify-between items-center mb-4">
          <h4 className="text-sm font-bold text-stone-700">?∂ÂÖ•?éÁ¥∞ </h4>
          <GlassButton onClick={() => onAddIncome(owner)} className="px-2 py-1 text-xs" variant="ghost">?∞Â?</GlassButton>
        </div>
        <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1 scrollbar-hide">
          ...
          {incomes.length === 0 && <div className="text-center text-xs text-stone-300 py-4">Â∞öÁÑ°?∂ÂÖ•Á¥Ä??/div>}
        </div>
      </div>
    </div>
  );
    ...
    <div className="space-y-8 pb-24 animate-in fade-in duration-500">
      ...
      <div className={`${GLASS_CARD} p-6 border-l-4 ${isOverBudget ? 'border-rose-400' : 'border-emerald-400'}`}>
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="min-w-0 flex-1">
            <h2 className="text-xs font-bold text-stone-400 uppercase tracking-widest">Âπ¥Â∫¶Á∏ΩÈ?ÁÆ?/h2>
        ...
        <div className="flex flex-row gap-4 w-full">
          <div className="bg-stone-50/50 rounded-xl p-3 flex-1 overflow-hidden">
            <div className="text-[10px] text-stone-400 font-bold uppercase mb-1">Â∑≤Ëä±Ë≤?/div>
          ...
          <div className={`rounded-xl p-3 flex-1 overflow-hidden ${isOverBudget ? 'bg-rose-50/50' : 'bg-emerald-50/50'}`}>
            <div className={`text-[10px] font-bold uppercase mb-1 ${isOverBudget ? 'text-rose-400' : 'text-emerald-400'}`}>{isOverBudget ? 'Ë∂ÖÊîØ' : '?©È?'}</div>
      ...
      <section>
        <div className="flex items-center gap-2 mb-4 px-1">
          ...
          <div><h2 className="text-lg font-bold text-stone-800 leading-tight">?àÂ∫¶?êÁ?</h2><p className="text-xs text-stone-400 font-bold tracking-wide uppercase">Á∂ìÂ∏∏?ßÊîØ??/p></div>
        </div>
        <div className={`${GLASS_CARD} p-5 mb-4 relative overflow-hidden border-l-4 border-stone-400`}>
          <BudgetProgressBar current={monthlyStats.totalUsed} total={monthlyStats.totalBudget} label="?¨Ê?Á∏ΩÂâ©È§? colorTheme="slate" />
        </div>
      ...
      <section>
        <div className="flex items-center gap-2 mb-4 px-1 mt-10">
          ...
          <div><h2 className="text-lg font-bold text-stone-800 leading-tight">Âπ¥Â∫¶?êÁ?</h2><p className="text-xs text-stone-400 font-bold tracking-wide uppercase">Âπ¥Â∫¶?πÂà•?ØÂá∫</p></div>
        </div>
          ...
          return (
            <div>
              ...
              <div className="flex justify-between text-xs text-stone-400">
                <span>{hasShares ? `${Number(item.shares).toLocaleString()}??x $${Number(item.amount).toLocaleString()}` : `$${Number(item.amount).toLocaleString()}`}{rate !== 1 && ` (?ØÁ? ${rate})`}</span>
              ...
              {diff !== null && (
                <div className={`mt-1 text-[10px] font-bold ${diff > 0 ? 'text-emerald-500' : 'text-rose-500'} flex items-center gap-1`}>
                  {diff > 0 ? '?•Ê?Ë≥?ç≤?? +' : '?•Ê?Ë≥?ôß?? '}${Math.abs(Math.round(diff)).toLocaleString()}
                  {diff > 0 ? '(?ÅËÇ°?≤Âà©)' : '(?ÅËÇ°?ßÊ?)'}
                </div>
              )}
            </div>
          );
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    calendarCells.push(
      <div key={day} onClick={() => setSelectedDay(day)} className={`h-24 border-t border-r border-stone-100/50 p-1 flex flex-col justify-between transition-colors cursor-pointer relative ${isSelected ? 'bg-stone-50/50 shadow-inner' : 'bg-white/30'} ${day % 7 === 0 ? 'border-r-0' : ''}`}>
        ...
        {dayTotal > 0 && (<div className="mb-1 flex flex-col items-end px-1 w-full"><span className="text-[10px] text-stone-400 font-medium">Á∏ΩË?</span><span className={`text-[10px] font-bold truncate w-full text-right ${dayTrans.some(t => t.type === 'annual') ? 'text-amber-600' : 'text-stone-600'}`}>${dayTotal.toLocaleString()}</span></div>)}
  ...
  return (
    <div className="pb-24 animate-in fade-in duration-300 relative">
      ...
      <div className={`${GLASS_CARD} p-0 border border-stone-100`}>
        <div className="grid grid-cols-7 bg-stone-50/50 border-b border-stone-100 rounded-t-3xl overflow-hidden">{['??, '‰∏Ä', '‰∫?, '‰∏?, '??, '‰∫?, '??].map(d => (<div key={d} className="py-2 text-center text-xs font-bold text-stone-400 uppercase tracking-wider">{d}</div>))}</div>
          ...
          <div className={`${GLASS_CARD} p-4`}>
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="text-sm font-bold text-stone-700">{selectedTrans.length} Á≠ÜÊ?Ë≤?/div>
                </div>
              </div>
              <div className="text-right">
            ...
            {selectedTrans.length === 0 ? (
              <div className="text-center py-6 text-stone-400 text-sm">?∂Êó•?°Ê?Ë≤ªÁ???/div>
            ) : (
...
const IncomeView = ({ incomes, salaryHistory, onAddSalary, onDeleteSalary, onDeleteIncome, onAddIncome, onEditSalary, selectedDate }) => {
  ...
  return (
    <div className="space-y-6 pb-24 animate-in fade-in">
      <CleanSummaryCard title="Âπ¥Â∫¶Á∏ΩÊî∂?? value={(myselfTotal + partnerTotal).toLocaleString()} subValue={`${currentYear}Âπ¥Â∫¶`} icon={Wallet} />
      <div className="flex flex-col gap-6">
        <PersonColumn name="Â£´Á?" owner="myself" incomes={myselfIncomes} total={myselfTotal} history={myselfHistory} icon={User} onAddSalary={onAddSalary} onDeleteSalary={onDeleteSalary} onDeleteIncome={onDeleteIncome} onAddIncome={onAddIncome} onEditSalary={onEditSalary} variant="blue" />
        <PersonColumn name="‰Ω≥Ê¨£" owner="partner" incomes={partnerIncomes} total={partnerTotal} history={partnerHistory} icon={Heart} onAddSalary={onAddSalary} onDeleteSalary={onDeleteSalary} onDeleteIncome={onDeleteIncome} onAddIncome={onAddIncome} onEditSalary={onEditSalary} variant="rose" />
      </div>
    </div>
  );
};
...
const PartnerView = ({ partnerTransactions, onDelete, onAdd, onEdit }) => {
  ...
  return (
    <div className="space-y-6 pb-24 animate-in fade-in">
      <CleanSummaryCard title="‰Ω≥Ê¨£?≤Ë?Á∏ΩË¶Ω" value={balance.toLocaleString()} subValue={`?ïÂÖ• $${totalSavings.toLocaleString()} - ?ØÂá∫ $${totalExpenses.toLocaleString()}`} icon={PiggyBank} trend={balance > 0 ? 'Ê≠???? : 'Ë≤†Ê???} variant="emerald" />
      <div className="flex flex-col gap-4">
        <div onClick={() => setIsHistoryOpen(!isHistoryOpen)} className="flex justify-between items-center px-1 cursor-pointer select-none hover:bg-stone-50/50 p-2 rounded-xl transition-colors">
          <h3 className="font-bold text-stone-700 flex items-center gap-2"><Wallet className="w-4 h-4" /> Ë≥áÈ?ËÆäÂ?Á¥Ä??{isHistoryOpen ? <ChevronUp className="w-3 h-3 text-stone-400" /> : <ChevronDown className="w-3 h-3 text-stone-400" />}</h3>
          <GlassButton onClick={(e) => { e.stopPropagation(); onAdd({}); }}><Plus className="w-3 h-3" /> ?∞Â?Á¥Ä??/GlassButton>
        </div>
        {isHistoryOpen && (partnerTransactions.length === 0 ? (<div className={`${GLASS_CARD} flex flex-col items-center justify-center h-48 text-stone-300 border-dashed`}><PiggyBank className="w-12 h-12 mb-2 opacity-20" /><p className="text-sm">Â∞öÁÑ°?≤Ë?Á¥Ä??/p></div>) : (groupedTransactions.map(([year, txs]) => (<PartnerYearGroup key={year} year={year} transactions={txs} onDelete={onDelete} onEdit={onEdit} />))))}
      </div>
    </div>
  );
};
    ...
    <div className="space-y-6 pb-24 animate-in fade-in">
      <Card>
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-lg font-bold text-stone-800">{isCompareMode ? 'Âπ¥Â∫¶?ØÂá∫ÊØîË?' : 'Âπ¥Â∫¶?ØÂá∫?ÜÊ?'}</h2>
          <div className="flex items-center gap-2">
            {!isCompareMode && (
              <button
            ...
            <button onClick={() => { setIsCompareMode(!isCompareMode); setSelectedMonth(null); }} className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold transition-colors ${isCompareMode ? 'bg-stone-800 text-white' : 'bg-stone-100 text-stone-600'}`}>
              {isCompareMode ? <ToggleRight className="w-4 h-4" /> : <ToggleLeft className="w-4 h-4" />} ÊØîË?Ê®°Â?
            </button>
          </div>
        </div>
        ...
        <div className="flex gap-4 mb-6">
          <div className="flex-1">
            <label className="text-[10px] text-stone-400 font-bold uppercase mb-1 block">{isCompareMode ? '‰∏ªÂπ¥‰ª? : '?∏Ê?Âπ¥‰ªΩ'}</label>
          ...
          {isCompareMode && (<div className="flex-1 animate-in slide-in-from-right-2 duration-200"><label className="text-[10px] text-stone-400 font-bold uppercase mb-1 block">Â∞çÊ?Âπ¥‰ªΩ</label><select value={compareYear} onChange={(e) => setCompareYear(Number(e.target.value))} className={`w-full ${GLASS_INPUT} px-3 py-2 font-bold text-stone-500`}>{availableYears.map(y => (<option key={y} value={y}>{y}</option>))}</select></div>)}
        </div>
      ...
      {isCompareMode ? (
        <Card>
          <h3 className="text-sm font-bold text-stone-700 mb-4">ÊØèÊ?Â∑ÆÁï∞?ÜÊ? ({baseYear} vs {compareYear})</h3>
          <div className="space-y-3">
            {monthlyDiffs.map((item) => (
              <div key={item.month} className="flex justify-between items-center text-sm border-b border-stone-50 last:border-0 pb-2 last:pb-0">
                <span className="text-stone-500 w-8">{item.month}??/span>
        ...
        <>
          <Card>
              ...
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="font-bold text-stone-500 text-xs flex items-center gap-2 mb-1">
                    ...
                    {selectedMonth !== null ? `${baseYear}Âπ?${selectedMonth + 1}?à` : `${baseYear} Âπ¥Â∫¶`} ?ØÂá∫ÁµÑÊ?
                    {(selectedMonth !== null || selectedFilter !== null) && <button onClick={(e) => { e.stopPropagation(); setSelectedMonth(null); setSelectedFilter(null); }} className="text-[10px] bg-stone-100 px-2 py-0.5 rounded-full text-stone-500 hover:bg-stone-200 ml-2">?çË®≠ÁØ©ÈÅ∏</button>}
                  </h3>
            ...
            <div className="space-y-4">
              {breakdownData.length > 0 ? (
                ...
                <div className="text-center text-stone-400 py-6 text-sm">Ë©≤Ê?ÊÆµÂ??°ÊîØ?∫Ë???/div>
              )}
            </div>
          </Card>
          ...
          {(selectedMonth !== null || selectedFilter !== null) && (
            <div className="animate-in slide-in-from-bottom-4 duration-500">
              <h3 className="text-sm font-bold text-stone-500 mb-3 px-2">Ë©≥Á¥∞?éÁ¥∞ </h3>
              <div className="space-y-3">
  ...
  return (
    <div className="pb-24 animate-in fade-in">
      <div className="flex bg-stone-100 p-1 rounded-xl mb-6">
        <button onClick={() => setActiveTab('goals')} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${activeTab === 'goals' ? 'bg-white text-stone-800 shadow-sm' : 'text-stone-400'}`}>?ÆÊ?Ë¶èÂ?</button>
        <button onClick={() => setActiveTab('exchange')} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${activeTab === 'exchange' ? 'bg-white text-stone-800 shadow-sm' : 'text-stone-400'}`}>?õÂåØÁ¥Ä??/button>
      </div>
      {activeTab === 'goals' ? (
        <div className="space-y-4 animate-in slide-in-from-left-4 duration-300">
          <div className="flex justify-end mb-2"><GlassButton onClick={onAddYear}><Plus className="w-3 h-3" /> ?∞Â?Âπ¥‰ªΩ</GlassButton></div>
          {sortedGoals.length === 0 ? <div className="text-center text-stone-400 py-10">Â∞öÁÑ°Ë≥áÊ? (Âæ?022?ãÂ?)</div> : sortedGoals.map((goal, index) => { const prevGoal = sortedGoals[index + 1]; const prevTotal = prevGoal ? getActualTotal(prevGoal) : 0; return <StockGoalCard key={goal.id} yearData={goal} prevYearTotal={prevTotal} onUpdate={onUpdate} />; })}
        </div>
      ) : (
        <div className="space-y-4 animate-in slide-in-from-right-4 duration-300">
          <CleanSummaryCard title="Á¥ØË?Ë≤∑ÂÖ•ÁæéÈ?" value={totalUSD.toLocaleString()} subValue={`Âπ≥Â??ØÁ?: ${avgRate.toFixed(4)}`} icon={ArrowRightLeft} />
          <div className="flex justify-between items-center px-1 mb-2">
            <h3 className="text-sm font-bold text-stone-500 flex items-center gap-2"><ArrowRightLeft className="w-4 h-4" /> ‰∫§Ê??éÁ¥∞</h3>
            <GlassButton onClick={onAddExchangeClick}><Plus className="w-3 h-3" /> ?∞Â??õÂåØ</GlassButton>
          </div>
          <div className="space-y-2">
            {exchanges.length === 0 ? <div className="text-center text-stone-400 py-10">Â∞öÁÑ°?õÂåØÁ¥Ä??/div> : exchanges.map(item => (<ExchangeItem key={item.id} item={item} onDelete={onDeleteExchange} />))}
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div className="mb-10 animate-in fade-in">
      <div className="flex justify-between items-end mb-4 border-b border-stone-100 pb-2">
        ...
        <button onClick={handleSaveWrapper} className={`text-xs px-3 py-1.5 rounded-lg shadow-sm transition-all flex items-center gap-1.5 font-bold ${isSaving ? 'bg-emerald-50 text-emerald-600' : 'bg-stone-800 text-white hover:bg-stone-700'}`}>{isSaving ? <><Check className="w-3 h-3" /> Â∑≤ÂÑ≤Â≠?/> : '?≤Â?ËÆäÊõ¥'}</button>
      </div>
      <div className="space-y-4">
          ...
          <div key={gIdx} className={`${GLASS_CARD} overflow-hidden p-0`}>
            ...
            <div className="p-4 space-y-3">
              ...
              <div className="flex gap-2 mt-3 pt-2">
                <input id={`${idPrefix}-n-${gIdx}`} placeholder="?ÖÁõÆ?çÁ®±" className={`${GLASS_INPUT} w-full text-xs py-2 px-3`} />
      ...
      <div className="flex gap-2 mt-4">
        <input value={newGroupName} onChange={e => setNewGroupName(e.target.value)} placeholder="?∞Â?Áæ§Á??çÁ®±..." className={`${GLASS_INPUT} flex-1 text-sm shadow-sm`} />
...
const RecurringManagerModal = ({ isOpen, onClose, items, onSave, groups }) => {
  ...
  if (!isOpen) return null;
  return (
    <ModalWrapper title="?∫Â??ØÂá∫Ë®≠Â?" onClose={onClose}>
      <div className="space-y-4">
        <div className="bg-amber-50 p-3 rounded-xl border border-amber-100 flex gap-2">
          <span className="text-amber-600 font-bold shrink-0">?í°</span>
          <span className="text-xs text-amber-700">?ØË®≠ÂÆöÊ??àÁ??•Â∏≥??(1-31)?ÇËã•?∂Ê??°Ë©≤?•Ê? (Â¶???0??ÔºåÂ??™Â?Á¥Ä?ÑÊñºË©≤Ê??ÄÂæå‰?Â§©„Ä?/span>
        </div>
        ...
        {!isAdding && (
          <div className="space-y-2 max-h-[40vh] overflow-y-auto">
                ...
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  ...
                  <div className="flex flex-col min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-[9px] px-1.5 py-0.5 rounded-md font-bold ${item.payer === 'partner' ? 'bg-rose-100 text-rose-500' : 'bg-stone-100 text-stone-500'}`}>{item.payer === 'partner' ? '‰Ω≥Ê¨£' : 'Â£´Á?'}</span>
                    </div>
                    <span className="text-xs text-stone-400">ÊØèÊ? {item.day || 1} ????${Number(item.amount).toLocaleString()} ??{item.group}-{item.category}</span>
                  </div>
                </div>
            ...
            {localItems.length === 0 && <div className="text-center py-8 text-stone-400 text-sm">Â∞öÁÑ°Ë®≠Â??ÖÁõÆ</div>}
          </div>
        )}
        ...
        <div className="pt-4 border-t border-stone-100 space-y-3">
          {!isAdding ? (
            <button onClick={() => setIsAdding(true)} className="w-full py-3 bg-stone-50 text-stone-500 font-bold rounded-xl border border-stone-200 hover:bg-stone-100 transition-colors flex items-center justify-center gap-2">
              <Plus className="w-4 h-4" /> ?∞Â??ÖÁõÆ
            </button>
          ) : (
            <div className="bg-stone-50 p-3 rounded-xl border border-stone-100 space-y-3 animate-in slide-in-from-top-2">
              <div className="flex justify-between items-center mb-1">
                <h4 className="text-xs font-bold text-stone-400 uppercase">{editingId ? 'Á∑®ËºØ?ÖÁõÆ' : '?∞Â??ÖÁõÆ'}</h4>
                <button onClick={handleCancelEdit} className="text-stone-400 hover:text-stone-600"><X className="w-4 h-4" /></button>
              </div>

              <div className="grid grid-cols-6 gap-2">
                <input placeholder="?çÁ®±" value={newItem.name} onChange={e => setNewItem({ ...newItem, name: e.target.value })} className={`${GLASS_INPUT} px-3 py-2 text-sm col-span-4`} />
                <div className="col-span-2 flex items-center bg-white/50 rounded-xl border border-stone-200 px-2 focus-within:ring-2 focus-within:ring-stone-400 focus-within:border-transparent transition-all">
                  <span className="text-xs text-stone-500 shrink-0 font-bold">ÊØèÊ?</span>
                  <input
                    type="number"
                    min="1"
                    max="31"
                    ...
                    placeholder="1"
                  />
                  <span className="text-xs text-stone-500 shrink-0 font-bold">??/span>
                </div>
                <input type="number" placeholder="?ëÈ?" value={newItem.amount} onChange={e => setNewItem({ ...newItem, amount: e.target.value })} className={`${GLASS_INPUT} px-3 py-2 text-sm col-span-6`} />
                ...
                <div className="col-span-6 flex gap-2 pt-1">
                  <button onClick={() => setNewItem({ ...newItem, payer: 'myself' })} className={`flex-1 py-2 rounded-xl text-xs font-bold transition-all border ${newItem.payer === 'myself' ? 'bg-blue-50 border-blue-200 text-blue-600' : 'bg-white border-stone-200 text-stone-400'}`}>Â£´Á?</button>
                  <button onClick={() => setNewItem({ ...newItem, payer: 'partner' })} className={`flex-1 py-2 rounded-xl text-xs font-bold transition-all border ${newItem.payer === 'partner' ? 'bg-rose-50 border-rose-200 text-rose-500' : 'bg-white border-stone-200 text-stone-400'}`}>‰Ω≥Ê¨£</button>
                </div>
                ...
                <button onClick={handleSaveItem} disabled={!newItem.name || !newItem.amount} className="col-span-6 bg-stone-800 text-white py-3 rounded-xl font-bold flex items-center justify-center gap-1 disabled:opacity-50 mt-2">
                  ...
                  {editingId ? 'Á¢∫Ë?‰øÆÊîπ' : 'Á¢∫Ë??∞Â?'}
                </button>
              </div>
            </div>
          )}
        </div>
        {!isAdding && <button onClick={handleSave} className="w-full bg-stone-800 text-white py-3 rounded-xl font-bold shadow-lg mt-4">?≤Â?Ë®≠Â?</button>}
      </div>
    </ModalWrapper>
  );
};
...
const RecurringConfirmModal = ({ isOpen, onClose, items, onConfirm, onSkip }) => {
  ...
  return (
    <ModalWrapper title="?¨Ê??∫Â??ØÂá∫Á¢∫Ë?" onClose={onClose}>
      <div className="space-y-4">
        <div className="text-center py-4">
          <div className="text-sm text-stone-500 mb-1">Ê™¢Ê∏¨?∞Êñ∞?ÑÊ?‰ªΩÔ??ØÂê¶?†ÂÖ•‰ª•‰??∫Â??ØÂá∫Ôº?/div>
        ...
        <div className="grid grid-cols-1 gap-3 pt-2">
          <button onClick={onConfirm} className="w-full bg-stone-800 text-white py-3 rounded-xl font-bold shadow-lg flex items-center justify-center gap-2">
            <CheckCircle2 className="w-4 h-4" /> Á¢∫Ë??•Â∏≥
          </button>
...
export default function App() {
  ...
  const [confirmModal, setConfirmModal] = useState({ isOpen: false, message: '', title: 'Á¢∫Ë?', confirmText: 'Á¢∫Â?', confirmColor: 'bg-stone-800', onConfirm: () => { } });
  ...
  const [newIncome, setNewIncome] = useState({ amount: '', category: '?™Ê∞¥', owner: 'myself', date: getTodayString(), note: '' });
  ...
  const withSubmission = async (action) => {
    ...
    setIsSubmitting(true);
    try { await action(); } catch (e) { console.error(e); alert('?ºÁ??ØË™§: ' + e.message); } finally { setIsSubmitting(false); }
  };

  const requestConfirmation = ({ message, title = 'Á¢∫Ë?', confirmText = 'Á¢∫Â?', confirmColor = 'bg-stone-800', onConfirm }) => {
    setConfirmModal({ isOpen: true, message, title, confirmText, confirmColor, onConfirm });
  };
  const requestDelete = (message, action) => requestConfirmation({ message, title: 'Á¢∫Ë??™Èô§', confirmText: '?™Èô§', confirmColor: 'bg-rose-500', onConfirm: action });
  ...
  const deleteTransaction = (id) => requestDelete("Á¢∫Â??™Èô§Ê≠§Á??ØÂá∫Á¥Ä?ÑÔ?", async () => deleteDoc(doc(db, 'artifacts', appId, 'ledgers', LEDGER_ID, 'transactions', id)));
  ...
  const handleDeleteIncome = (id) => requestDelete("Á¢∫Â??™Èô§Ê≠§Á??∂ÂÖ•Á¥Ä?ÑÔ?", async () => deleteDoc(doc(db, 'artifacts', appId, 'ledgers', LEDGER_ID, 'incomes', id)));
  ...
  const handleDeleteSalaryRecord = (id) => requestDelete("Á¢∫Â??™Èô§Ê≠§Ë™ø?™Á??ÑÔ?", async () => deleteDoc(doc(db, 'artifacts', appId, 'ledgers', LEDGER_ID, 'salary_history', id)));
  ...
  const deletePartnerTx = (id) => requestDelete("Á¢∫Â??™Èô§Ê≠§Á??≤Ë?/?ØÂá∫Á¥Ä?ÑÔ?", async () => deleteDoc(doc(db, 'artifacts', appId, 'ledgers', LEDGER_ID, 'partner_savings', id)));
  ...
  const deleteMortgageExp = (id) => requestDelete('?™Èô§Ê≠§È??ÆÔ?', () => deleteDoc(doc(db, 'artifacts', appId, 'ledgers', LEDGER_ID, 'mortgage_expenses', id)));
  ...
  const deleteMortgageAnalysis = (id) => requestDelete('?™Èô§Ê≠§È??ÆÔ?', () => deleteDoc(doc(db, 'artifacts', appId, 'ledgers', LEDGER_ID, 'mortgage_analysis', id)));
  ...
  const deleteMortgageFunding = (id) => requestDelete('?™Èô§Ê≠§È??ÆÔ?', () => deleteDoc(doc(db, 'artifacts', appId, 'ledgers', LEDGER_ID, 'mortgage_funding', id)));
  ...
  const handleDeleteExchange = (id) => requestDelete('?™Èô§Ê≠§Ê??ØÁ??ÑÔ?', () => deleteDoc(doc(db, 'artifacts', appId, 'ledgers', LEDGER_ID, 'usd_exchanges', id)));
  ...
  return (
    <div className="flex flex-col h-screen bg-stone-50 text-stone-800 font-sans overflow-hidden max-w-md mx-auto relative shadow-2xl">
      {/* Background Blobs - Nippon Colors: Ê°úÈ? (Sakura-nezumi), ?ΩËó§ (Shiro-fuji), ?•Ë? (Wakakusa) */}
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-50 flex animate-in slide-in-from-left duration-300">
          <div className="w-64 bg-white/95 backdrop-blur-xl h-full shadow-2xl p-6 relative border-r border-stone-100">
            ...
            <div className="mb-8 mt-2 px-2"><h1 className="text-xl font-bold text-stone-700 flex items-center gap-2"><img src={icon} className="w-8 h-8 rounded-lg shadow-md" alt="Logo" /> Ë®òÂ∏≥?©Ê?</h1><p className="text-xs text-stone-400 mt-1 pl-1">v1.0.0(Mick)</p></div>
        ...
        <div className="flex-1 flex justify-center">
          {(currentView === 'home' || currentView === 'income') ? (
            <div className="flex items-center gap-3 bg-white/50 rounded-full px-1 py-1 border border-white/40 shadow-sm backdrop-blur-sm">
              ...
              <span className="text-sm font-bold text-stone-700 min-w-[5rem] text-center select-none">{currentView === 'home' ? selectedDate.toLocaleString('zh-TW', { year: 'numeric', month: 'long' }) : `${selectedDate.getFullYear()}Âπ¥`}</span>
          ...
          <IncomeView
            ...
            onAddIncome={(own, item = null) => {
              if (item) {
                ...
                setNewIncome(prev => ({ ...prev, owner: own, amount: '', category: '?™Ê∞¥', note: '', date: selectedDate ? toLocalISOString(selectedDate) : getTodayString() }));
              }
        ...
        {currentView === 'calendar' && (
          <CalendarView
            ...
            onEdit={(item) => {
              ...
              if (!item.id) {
                alert('?ØË™§ÔºöÊ≠§‰∫§Ê?Ê≤íÊ??âÊ?IDÔºåÁÑ°Ê≥ïÁ∑®ËºØ„ÄÇË??çÊñ∞?¥Á??ÅÈù¢ÂæåÂ?Ë©¶„Ä?);
                return;
              }
        ...
        {currentView === 'settings' && (
          <div className="pb-24">
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6 flex items-start gap-3">
              ...
              <div>
                <h3 className="font-bold text-amber-800 text-sm">Ê≠?ú®Á∑®ËºØ {selectedDate.getFullYear()} Âπ¥Â∫¶?êÁ?</h3>
                <p className="text-xs text-amber-600 mt-1">Ê≠§Ë??ÑË??¥Â??ÉÂ??®Âà∞ {selectedDate.getFullYear()} Âπ¥Ô?‰∏çÊ?ÂΩ±Èüø?∂‰?Âπ¥‰ªΩ?ÑË®≠ÂÆö„Ä?/p>
              </div>
            </div>
            <GroupSettingsEditor title={`${selectedDate.getFullYear()}Âπ¥Ê?Â∫¶È?ÁÆóÈ?ÁΩÆ`} groups={settings.monthlyGroups} onSave={(g) => updateSettings(g, 'monthly')} idPrefix="monthly" />
            <GroupSettingsEditor title={`${selectedDate.getFullYear()}Âπ¥Âπ¥Â∫¶È?ÁÆóÈ?ÁΩÆ`} groups={settings.annualGroups} onSave={(g) => updateSettings(g, 'annual')} idPrefix="annual" />
          </div>
        )}
      </main>
      ...
      {/* --- Modals --- */}
      {isAddTxModalOpen && (
        <ModalWrapper title={editingId ? "Á∑®ËºØ?ØÂá∫" : "?∞Â??ØÂá∫"} onClose={() => { setIsAddTxModalOpen(false); setEditingId(null); setNewTrans({ amount: '0', type: 'monthly', group: '', category: '', note: '', date: getTodayString(), payer: 'myself' }); }}>
          {/* Ê™¢Êü•?ØÂê¶?âË®≠ÂÆöÈ?ÁÆóÁæ§ÁµÑÔ??•ÁÑ°?áÊ?Á§?*/}
          {(settings.monthlyGroups.length === 0 && settings.annualGroups.length === 0) ? (
            <div className="text-center py-10">
              <p className="text-stone-500 mb-4">Ë´ãÂ?Ë®≠Â??êÁ??ÜÈ?</p>
              <GlassButton onClick={() => { setIsAddTxModalOpen(false); setCurrentView('settings'); }}>?çÂ?Ë®≠Â?</GlassButton>
            </div>
          ) : (
            <form onSubmit={handleAddTransaction} className="space-y-4">
              <div className="flex justify-end">
                <button type="button" onClick={() => setIsRecurringManagerOpen(true)} className="text-xs text-stone-500 underline flex items-center gap-1 hover:text-stone-800"><SettingsIcon className="w-3 h-3" />ÁÆ°Á??∫Â??ØÂá∫</button>
              </div>
              <CalculatorInput
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-stone-100/50 p-1 rounded-2xl flex">
                  <button type="button" onClick={() => setNewTrans({ ...newTrans, type: 'monthly', group: '' })} className={`flex-1 py-2 rounded-xl text-xs font-bold transition-all ${newTrans.type === 'monthly' ? 'bg-white shadow-sm text-stone-800' : 'text-stone-400'}`}>?àÂ∫¶</button>
                  <button type="button" onClick={() => setNewTrans({ ...newTrans, type: 'annual', group: '' })} className={`flex-1 py-2 rounded-xl text-xs font-bold transition-all ${newTrans.type === 'annual' ? 'bg-white shadow-sm text-stone-600' : 'text-stone-400'}`}>Âπ¥Â∫¶</button>
                </div>
                <div className="bg-stone-100/50 p-1 rounded-2xl flex">
                  <button type="button" onClick={() => setNewTrans({ ...newTrans, payer: 'myself' })} className={`flex-1 py-2 rounded-xl text-xs font-bold transition-all ${newTrans.payer === 'myself' ? 'bg-white shadow-sm text-blue-600' : 'text-stone-400'}`}>Â£´Á?</button>
                  <button type="button" onClick={() => setNewTrans({ ...newTrans, payer: 'partner' })} className={`flex-1 py-2 rounded-xl text-xs font-bold transition-all ${newTrans.payer === 'partner' ? 'bg-white shadow-sm text-rose-500' : 'text-stone-400'}`}>‰Ω≥Ê¨£</button>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-3 min-w-0">
                ...
                <div className="w-full relative">
                  <InputField value={newTrans.note} onChange={(e) => setNewTrans({ ...newTrans, note: e.target.value })} placeholder="?ôË®ª..." onFocus={(e) => {
              ...
              <GlassButton type="submit" disabled={isSubmitting} className="w-full py-4 text-base rounded-2xl mt-4 shadow-xl shadow-stone-300/50">{isSubmitting ? '?ïÁ?‰∏?..' : 'Á¢∫Ë??≤Â?'}</GlassButton>
            </form>
          )}
        </ModalWrapper>
      )}
      ...
      {isAddMortgageExpModalOpen && (
        <ModalWrapper title={mortgageExpType === 'down_payment' ? '?∞Â??≠Ê??úÊîØ' : '?∞Â??úÊîØÁ¥Ä??} onClose={() => setIsAddMortgageExpModalOpen(false)}>
          <form onSubmit={handleAddMortgageExp} className="space-y-4">
            <InputField label="?ÖÁõÆ?çÁ®±" value={newMortgageExp.name} onChange={e => setNewMortgageExp({ ...newMortgageExp, name: e.target.value })} autoFocus required />
            <InputField label="?ëÈ?" type="number" value={newMortgageExp.amount} onChange={e => setNewMortgageExp({ ...newMortgageExp, amount: e.target.value })} required />
            <InputField label="?•Ê?" type="date" value={newMortgageExp.date} onChange={e => setNewMortgageExp({ ...newMortgageExp, date: e.target.value })} required />
            {mortgageExpType === 'misc_appliances' && (<InputField label="?ÅÁ?" value={newMortgageExp.brand} onChange={e => setNewMortgageExp({ ...newMortgageExp, brand: e.target.value })} placeholder="?ÅÁ?" />)}
            <InputField label="?ôË®ª" value={newMortgageExp.note} onChange={e => setNewMortgageExp({ ...newMortgageExp, note: e.target.value })} />
            <GlassButton type="submit" disabled={isSubmitting} className="w-full py-4 text-base rounded-2xl mt-4">{isSubmitting ? '?ïÁ?‰∏?..' : '?≤Â?'}</GlassButton>
          </form>
        </ModalWrapper>
      )}

      {isAddMortgageFundingModalOpen && (
        <ModalWrapper title="?∞Â??≠Ê?Ê¨æ‰?Ê∫? onClose={() => setIsAddMortgageFundingModalOpen(false)}>
          <form onSubmit={handleAddMortgageFunding} className="space-y-4">
            <InputField label="Ë≥áÈ?‰æÜÊ?" value={newMortgageFunding.source} onChange={e => setNewMortgageFunding({ ...newMortgageFunding, source: e.target.value })} placeholder=" " autoFocus required />
            <InputField label="?°Á•®‰ª?¢º (?∏Â°´)" value={newMortgageFunding.symbol} onChange={e => setNewMortgageFunding({ ...newMortgageFunding, symbol: e.target.value })} placeholder=" " />
            <div className="flex gap-2">
              <div className="flex-1"><InputField label="?ëÈ?/?ÆÂÉπ" type="number" value={newMortgageFunding.amount} onChange={e => setNewMortgageFunding({ ...newMortgageFunding, amount: e.target.value })} required /></div>
              <div className="w-24"><InputField label="?ØÁ?" type="number" value={newMortgageFunding.rate} onChange={e => setNewMortgageFunding({ ...newMortgageFunding, rate: e.target.value })} placeholder="1.0" /></div>
            </div>
            <InputField label="?°Êï∏ (?∏Â°´)" type="number" value={newMortgageFunding.shares} onChange={e => setNewMortgageFunding({ ...newMortgageFunding, shares: e.target.value })} placeholder="0" />
            <InputField label="?•Ê?" type="date" value={newMortgageFunding.date} onChange={e => setNewMortgageFunding({ ...newMortgageFunding, date: e.target.value })} required />
            <InputField label="?ôË®ª" value={newMortgageFunding.note} onChange={e => setNewMortgageFunding({ ...newMortgageFunding, note: e.target.value })} />
            <GlassButton type="submit" disabled={isSubmitting} className="w-full py-4 text-base rounded-2xl mt-4">{isSubmitting ? '?ïÁ?‰∏?..' : '?≤Â?'}</GlassButton>
          </form>
        </ModalWrapper>
      )}
      ...
      {isAddMortgageAnalysisModalOpen && (
        <ModalWrapper title="?∞Â??íÁ?Ë©¶Á??ÖÁõÆ" onClose={() => setIsAddMortgageAnalysisModalOpen(false)}>
          <form onSubmit={handleAddMortgageAnalysis} className="space-y-4">
            <InputField label="?ÖÁõÆ?çÁ®±" value={newMortgageAnalysis.name} onChange={e => setNewMortgageAnalysis({ ...newMortgageAnalysis, name: e.target.value })} autoFocus required />
            <InputField label="?ëÈ?" type="number" value={newMortgageAnalysis.amount} onChange={e => setNewMortgageAnalysis({ ...newMortgageAnalysis, amount: e.target.value })} required />
            <GlassButton type="submit" disabled={isSubmitting} className="w-full py-4 text-base rounded-2xl mt-4">{isSubmitting ? '?ïÁ?‰∏?..' : '?≤Â?'}</GlassButton>
          </form>
        </ModalWrapper>
      )}
      ...
      {isAddIncomeModalOpen && (
        <ModalWrapper title={editingId ? "Á∑®ËºØ?∂ÂÖ•" : "?∞Â??∂ÂÖ•"} onClose={() => { setIsAddIncomeModalOpen(false); setEditingId(null); setNewIncome({ amount: '', category: '?™Ê∞¥', owner: 'myself', date: getTodayString(), note: '' }); }}>
          <form onSubmit={handleAddIncome} className="space-y-6">
            <InputField label="?ëÈ?" type="number" value={newIncome.amount} onChange={(e) => setNewIncome({ ...newIncome, amount: e.target.value })} autoFocus required />
            <div className="space-y-1.5"><label className="block text-xs font-bold text-stone-400 uppercase tracking-wider ml-1">?ÜÈ?</label><div className="relative"><select value={newIncome.category} onChange={(e) => setNewIncome({ ...newIncome, category: e.target.value })} className={`w-full p-4 ${GLASS_INPUT} text-stone-800 font-medium outline-none appearance-none text-sm`}>{INCOME_CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}</select></div></div>
            <InputField label="?•Ê?" type="date" value={newIncome.date} onChange={(e) => setNewIncome({ ...newIncome, date: e.target.value })} required />
            <InputField label="?ôË®ª" value={newIncome.note} onChange={(e) => setNewIncome({ ...newIncome, note: e.target.value })} placeholder="?ôË®ª..." />
            <GlassButton type="submit" disabled={isSubmitting} className="w-full py-4 text-base rounded-2xl mt-4">{isSubmitting ? '?ïÁ?‰∏?..' : 'Á¢∫Ë??•Â∏≥'}</GlassButton>
          </form>
        </ModalWrapper>
      )}

      {isAddPartnerTxModalOpen && (
        <ModalWrapper title={editingId ? "Á∑®ËºØË≥áÈ?Á¥Ä?? : "?∞Â?Ë≥áÈ?Á¥Ä??} onClose={() => { setIsAddPartnerTxModalOpen(false); setEditingId(null); setNewPartnerTx({ amount: '', type: 'saving', date: getTodayString(), note: '' }); }}>
          <form onSubmit={handleAddPartnerTx} className="space-y-6">
            <div className="flex gap-2">
              <GlassButton onClick={() => setNewPartnerTx({ ...newPartnerTx, type: 'saving' })} variant={newPartnerTx.type === 'saving' ? 'success' : 'ghost'} className="flex-1">Â≠òÂÖ•Ë≥áÈ?</GlassButton>
              <GlassButton onClick={() => setNewPartnerTx({ ...newPartnerTx, type: 'expense' })} variant={newPartnerTx.type === 'expense' ? 'danger' : 'ghost'} className="flex-1">?ØÂá∫/?êÈ?</GlassButton>
            </div>
            <div>
              <label className="block text-xs font-bold text-stone-400 uppercase tracking-wider ml-1 mb-1">?ëÈ?</label>
            ...
            <InputField label="?•Ê?" type="date" value={newPartnerTx.date} onChange={(e) => setNewPartnerTx({ ...newPartnerTx, date: e.target.value })} required />
            <InputField label="?ôË®ª" value={newPartnerTx.note} onChange={(e) => setNewPartnerTx({ ...newPartnerTx, note: e.target.value })} placeholder="Ë≥áÈ??®ÈÄ?.." />
            <GlassButton type="submit" disabled={isSubmitting} className="w-full py-4 text-base rounded-2xl mt-4">{isSubmitting ? '?ïÁ?‰∏?..' : 'Á¢∫Ë??≤Â?'}</GlassButton>
          </form>
        </ModalWrapper>
      )}
      ...
      {isAddSalaryModalOpen && (
        <ModalWrapper title="Ë™øËñ™Á¥Ä?? onClose={() => setIsAddSalaryModalOpen(false)}>
          <form onSubmit={handleAddSalaryRecord} className="space-y-6">
            <InputField label="?∞Ëñ™Ë≥áÈ?È°? type="number" value={newSalaryRecord.amount} onChange={(e) => setNewSalaryRecord({ ...newSalaryRecord, amount: e.target.value })} autoFocus required />
            <InputField label="?üÊ??•Ê?" type="date" value={newSalaryRecord.date} onChange={(e) => setNewSalaryRecord({ ...newSalaryRecord, date: e.target.value })} required />
            <GlassButton type="submit" disabled={isSubmitting} className="w-full py-4 text-base rounded-2xl mt-4">{isSubmitting ? '?ïÁ?‰∏?..' : '?≤Â?Ë™øËñ™'}</GlassButton>
          </form>
        </ModalWrapper>
      )}

      {isAddExchangeModalOpen && (
        <ModalWrapper title="?∞Â??õÂåØÁ¥Ä?? onClose={() => setIsAddExchangeModalOpen(false)}>
          <form onSubmit={handleAddExchange} className="space-y-4">
            ...
            <InputField label="?õÂÖ•ÁæéÈ? (USD)" type="number" value={newExchange.usdAmount} onChange={e => setNewExchange({ ...newExchange, usdAmount: e.target.value })} autoFocus required />
            <InputField label="?ØÁ? (TWD/USD)" type="number" value={newExchange.rate} onChange={e => setNewExchange({ ...newExchange, rate: e.target.value })} required />
            <InputField label="?•Ê?" type="date" value={newExchange.date} onChange={e => setNewExchange({ ...newExchange, date: e.target.value })} required />
            <GlassButton type="submit" disabled={isSubmitting} className="w-full py-4 text-base rounded-2xl mt-4">{isSubmitting ? '?ïÁ?‰∏?..' : 'Á¢∫Ë?Á¥Ä??}</GlassButton>
          </form>
        </ModalWrapper>
      )}
